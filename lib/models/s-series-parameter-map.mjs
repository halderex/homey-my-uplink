...
const sSeriesParameterMap = {
    ...
    // The pump's OWN per-function energy books, as myUplink reports them — deliberately not
    // `meter_power.*` so Homey Energy never accounts them: they are a read-only reference for
    // cross-checking the derived split against the pump's own arithmetic, not a meter.
    // Each role monitors only its own pair, so the shared capability ids never collide.
    // Note these are *excl. additional heater*, so the honest local-Modbus equivalent is
    // (2291 − 2299) for heating and (2293 − 2301) for hot water.
    [SSeriesParameterIds.LIFETIME_HEATING_USED_ENERGY]: {
        capabilityName: "myuplink_used_energy",
        parameterName: "Used Energy - Heating (excl. additional heater)",
        type: "number"
    },
    [SSeriesParameterIds.LIFETIME_HOT_WATER_USED_ENERGY]: {
        capabilityName: "myuplink_used_energy",
        parameterName: "Used Energy - Hot Water (excl. additional heater)",
        type: "number"
    },
    [SSeriesParameterIds.HEATING_PRODUCED_ENERGY]: {
        capabilityName: "myuplink_produced_energy",
        parameterName: "Produced Energy - Heating (incl. additional heater)",
        type: "number"
    },
    [SSeriesParameterIds.HOT_WATER_PRODUCED_ENERGY]: {
        capabilityName: "myuplink_produced_energy",
        parameterName: "Produced Energy - Hot Water (incl. additional heater)",
        type: "number"
    },
